...
function parseDataTable(attrs) {
  ...
  return expand({ cells: rows, head: p.head ?? 1, align: [], layout: p.layout, widths: p.widths,
    hideHeader: Boolean(p.hideHeader), noFirstCol: Boolean(p.noFirstCol) });
}

// 紧凑行（省略被覆盖格）→ 稠密矩阵（null 占位）
function expand({ cells, head, align, layout, widths, hideHeader, noFirstCol }) {
  const out = [];
  const put = (r, c, v) => {
  ...
  return { cells: out, head, align, ...(layout && { layout }), ...(widths && { widths }),
...
const isPlain = (grid) =>
  !grid.layout && !grid.widths?.some(Boolean) && !grid.hideHeader && !grid.noFirstCol &&
  grid.head === 1 &&
...
function toDataTable({ cells, head, align, layout, widths, hideHeader, noFirstCol }) {
  ...
  return `<DataTable${headAttr}${layoutAttr}${widthsAttr}${displayAttrs} data={[\n${rows.join('\n')}\n]}/>`;
}
